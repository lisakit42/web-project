    ...
    return <div id={`programm-${props.programmInfo.Id}`} className={'TileWrapper ' + (deleted ? 'deleted' : '')} onClick={() => { if (!hoverDelete) navigate(`./programm-edit-page/${props.programmInfo.Id}`) }} >
        ...
        {deletePrevent ? <div className="TileWrapper deletePreventWindow" onMouseOut={() => { hoverDelete = false }} onMouseOver={() => { hoverDelete = true }}>
            ...
            <div className="deletePreventButtons">
                <button className="accept" onClick={() => {setLoading(true); axios.delete(deleteProgrammApi).then(() => { props.gapInc(); setLoading(false); setDeleted(true); setTimeout(() => {document.getElementById(`programm-${props.programmInfo.Id}`).remove();}, 200)}).catch(err => console.log(err))}}>Да</button>
